...
- [IApiPlugin](/docspace/plugins-sdk/usage-sdk/coding-plugin/plugin-types/apiplugin.md)。用于访问 DocSpace API，以下载源文件并将转换后的 Markdown 文件上传到同一文件夹。
- [IContextMenuPlugin](/docspace/plugins-sdk/usage-sdk/coding-plugin/plugin-types/contextmenuplugin.md) 和 [IContextMenuItem](/docspace/plugins-sdk/usage-sdk/coding-plugin/plugin-items/contextmenuitem.md)。用于实现上下文菜单操作。该插件会为 `.docx`、`.txt` 和 `.html` 文件添加 **Convert to Markdown** 项目。
