...
执行 API 请求时，需要通过 HTTP 标头传递身份验证信息。身份验证要求在 HTTP 请求的 **Authorization** 标头中使用令牌。

**身份验证令牌**是以下格式的字符串序列："ASC *pkey*:*datetime*:*hash*"，其中

- **pkey** - 随机字符串，
- **datetime** - 当前 UTC 日期和时间，格式为 "*yyyyMMddHHmmss*"，
- **hash** - 以 "*datetime*\n*pkey*" 格式字符串的哈希值。
