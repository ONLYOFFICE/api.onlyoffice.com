...
将 UTF8 格式的参数设置到构建器类中，该参数可传递到 [CDocBuilder.ExecuteCommand](./ExecuteCommand.md) 方法之外的程序，即可以作为运行 ONLYOFFICE Document Builder 可执行文件时的附加属性，也可以作为程序代码的一部分，但不包含在文档文件脚本中。

> 请注意，对于 `.docbuilder` 文件，不会显式使用 `CDocBuilder.SetProperty` 方法。相反，该参数本身会作为可执行文件的附加属性使用。见下面的示例。
...
| **名称** | **数据类型** | **描述** |
| -------- | ------------- | -------- |
| sParam | const char* | UTF8 格式的参数名称，其值始终为 `--argument`。 |
| sValue | const char* | UTF8 格式的参数值，将在文档中使用。 |
...
| --argument | const wchar_t* | "" | 发送到所有已打开的 JS 上下文的全局参数的 JSON 参数。 |
