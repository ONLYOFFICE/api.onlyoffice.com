...
4. **执行。** 所选函数运行：向 AI 模型发送请求，并使用 [Office API](/docs/office-api/get-started/overview) 将结果应用到文档中。
...
- **函数执行**：实现核心逻辑，包括向 AI 模型发送请求以及使用 [Office API](/docs/office-api/get-started/overview) 操作文档内容。
...
:::note
为了确保在请求执行后可以回滚整个更改块，我们在 `commentText` 函数中使用了 [StartAction](/docs/plugins/interacting-with-editors/document-api/Methods/StartAction) 和 [EndAction](/docs/plugins/interacting-with-editors/document-api/Methods/EndAction) 方法。
:::
