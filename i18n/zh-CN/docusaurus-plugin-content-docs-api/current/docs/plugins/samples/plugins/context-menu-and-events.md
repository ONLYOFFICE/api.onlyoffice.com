...
将以下代码分别保存到 *config.json*、*index.html* 和 *code.js* 文件中。从[此插件](https://github.com/ONLYOFFICE/sdkjs-plugins/tree/master/example_customfields_add/resources)下载包含图标的 *resources* 文件夹。将所有文件和该文件夹放入插件目录中，并根据以下说明进行安装：[桌面版](/docs/plugins/development-workflow/installing-and-testing/desktop-editors-installation)、[本地部署版](/docs/plugins/development-workflow/installing-and-testing/docs-on-premises-installation)、或[云端版](/docs/plugins/development-workflow/installing-and-testing/cloud-saas-installation)。
