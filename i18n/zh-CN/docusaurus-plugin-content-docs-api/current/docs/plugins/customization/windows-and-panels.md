...
1. 在[插件代码](../interacting-with-editors/overview/overview.md)文件中，指定类似于插件配置中 [variations](../fundamentals/configuration/configuration.md#variations) 的模态窗口 / 面板设置。
   ...
   **参数说明**:
   ...
   | variations.type        | `string`        | "window"                                                                                                           | 插件的[type](../fundamentals/configuration/configuration.md#variationstype)。使用 `"window"` 调取模态窗口，`"panel"` 调取左侧面板，`"panelRight"` 调取右侧面板。                                                                                     |
