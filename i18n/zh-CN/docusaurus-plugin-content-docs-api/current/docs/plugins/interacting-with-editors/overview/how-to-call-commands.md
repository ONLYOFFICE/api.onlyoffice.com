...
该对象用于修改对象数据，并在执行**调用命令**方法时传递附加参数。**info** 是一个辅助对象，在插件运行时可用。它存储所有与当前使用插件的编辑器相关的信息（所使用的[编辑器类型](#example-for-the-editortype-parameter)：文本文档、电子表格、演示文稿、PDF 等），以及 OLE 对象的附加设置（如宽度、高度、OLE 对象矢量绘图的毫米与像素比等参数）。

例如，如果文档内容发生变更并需要重新计算，则必须将 [recalculate](#example-for-the-recalculate-parameter) 参数设置为 true。此操作是必要的，因为重新计算过程是异步的。此外，还可能需要上传其他数据（例如字体等）。
