...
:::note
为了确保在请求执行后可以回滚整个更改块，我们在 `commentText` 函数中使用了 [StartAction](../plugins/interacting-with-editors/text-document-api/Methods/StartAction.md) 和 [EndAction](../plugins/interacting-with-editors/text-document-api/Methods/EndAction.md) 方法。
:::
