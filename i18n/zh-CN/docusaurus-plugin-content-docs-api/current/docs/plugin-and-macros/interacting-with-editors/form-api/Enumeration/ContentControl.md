...
| FormValue | string \| boolean \| Date | 表单字段的当前值。仅在内容控件为表单字段时显示。 |
...
| RadioGroup | string | 单选按钮的组键。仅在内容控件为单选按钮表单字段时显示。 |
| Shd | Object | 内容控件的底纹颜色。仅在设置了底纹颜色时显示。 |
| Tag | string | 分配给内容控件的标签。同一标签可以分配给多个内容控件，以便在代码中引用它们。 |
