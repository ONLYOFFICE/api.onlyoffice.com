...
| options | [&#123;value: string, label: string&#125;](../../text-document-api/&#123;value: string, label: string&#125;/&#123;value: string, label: string&#125;.md)[] \| boolean[] | 字段的可用选项列表。适用于复选框、单选按钮组、下拉列表和组合框。对于**复选框**为 **[true, false]**。对于**单选按钮**、**下拉列表**和**组合框**，每个条目包含一个 **value**（存储值）和一个 **label**（显示文本）。 |
| role | string | 表单角色。 |
