...
| arg2 | 可选 | [ApiRange](../../ApiRange/ApiRange.md) \| [ApiName](../../ApiName/ApiName.md) \| number |  | 确定返回值类型的数字：**1** - 返回 1（星期日）到 7（星期六）的数字；**2** - 返回 1（星期一）到 7（星期日）的数字；**3** - 返回 0（星期一）到 6（星期日）的数字。 |
