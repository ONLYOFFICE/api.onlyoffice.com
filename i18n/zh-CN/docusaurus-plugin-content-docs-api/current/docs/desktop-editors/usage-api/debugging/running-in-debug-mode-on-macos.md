...
使用终端在 macOS 上以调试模式运行 ONLYOFFICE 桌面编辑器：

1. 按下 **Command+空格**。

2. 在弹出的 **Spotlight** 窗口中，在搜索栏输入 *Terminal*。

3. 选择 **终端** 选项，终端将会打开。

4. 在命令行中，输入应用程序的路径，加一个空格，然后输入 *--ascdesktop-support-debug-info* 标志：
...
要开始在调试模式下工作，右键点击顶部工具栏上的任何可编辑字段（例如，字体列表组合框）并点击 **检查元素**，或者在文档的任意位置点击并按下 **F1**。
