...
1. 更改 *config/default.json* 文件中文档服务器的地址：
   ...
   其中 **documentserver** 是安装了 **ONLYOFFICE 文档** 的服务器的名称。您可以[注册](https://www.onlyoffice.com/zh/docs-registration?from=api)一个免费的 ONLYOFFICE 云，并使用其公共 IP 地址或公共 DNS，这些地址或 DNS 可以在云控制台的**实例**部分找到。
