...
PDF 表单在表单元数据方面与标准 PDF 文件有所不同。这种差异决定了打开文件的编辑器类型：是表单编辑器还是标准 PDF 编辑器。

为了区分标准 PDF 文件和 PDF 表单，可以使用检查函数。这些函数会读取文件的前 300 个字节，并检查其中是否存在`"ONLYOFFICEFORM"`。如果`true`，那么这就是一个扩展的 PDF 文件。否则，它就是一个标准的 PDF 文件。

:::请注意，
通过指定[document.isForm](../../usage-api/config/document/document.md#isform)参数，您可以加快文件的打开速度。
:::

在下面，您可以找到不同编程语言的检查函数。
