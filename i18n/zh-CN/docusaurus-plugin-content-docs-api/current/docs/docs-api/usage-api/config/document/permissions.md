...
允许在使用 [onRequestRestore](../events.md#onrequestrestore) 事件时显示*恢复*按钮。默认值为 **false**。自 5.5 版起已弃用，请改为添加 [onRequestRestore](../events.md#onrequestrestore) 字段。
...
允许在使用 [onRequestRename](../events.md#onrequestrename) 事件时显示 *重命名...* 按钮。默认值为 **false**。自 6.0 版起已弃用，请改为添加 [onRequestRename](../events.md#onrequestrename) 字段。
