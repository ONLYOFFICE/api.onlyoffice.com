...
定义是否可以将内容复制到剪贴板。如果参数设置为 `false`，则粘贴内容将仅在当前文档编辑器中可用。
...
定义文档是可以下载还是只能在线查看或编辑。如果下载权限设置为 `false`，则 **文件** 菜单中将不存在 **下载为...** 菜单选项。
...
定义是否可以打印文档。如果打印权限设置为 `false`，则 **文件** 菜单中将不存在 **打印** 菜单选项。
