...
1. 当用户打开表单文档时，将执行 [GetAllContentControls](/docs/plugins/interacting-with-editors/document-api/Methods/GetAllContentControls.md) 方法以从文档中收集所有内容控件。之后，执行 [GetFormValue](/docs/plugins/interacting-with-editors/form-api/Methods/GetFormValue.md) 方法来获取内容控件的值，并将其显示在自定义界面中：
...
2. 当用户从列表中选择用户名时，将执行 [GetFormsByTag](/docs/plugins/interacting-with-editors/form-api/Methods/GetFormsByTag.md) 方法，通过其标签收集所有表单，并使用 [SetFormValue](/docs/plugins/interacting-with-editors/form-api/Methods/SetFormValue.md) 方法为其设置相应的值：
...
3. 当用户编辑表单值时，会触发 [onChangeContentControl](/docs/plugins/interacting-with-editors/document-api/Events/onChangeContentControl.md) 事件，然后执行 [GetFormValue](/docs/plugins/interacting-with-editors/form-api/Methods/GetFormValue.md) 方法以获取更新的表单值并将其显示在自定义界面中：
