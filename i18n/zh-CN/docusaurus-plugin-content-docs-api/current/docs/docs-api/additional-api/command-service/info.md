...
| key       | string           | 必填 | 定义文档标识符，用于明确标识文档文件。                                                                                                                                                                                                                                                                                                     |
| users     | string[] | 必填 | 定义已打开文档进行编辑的用户ID列表。当文档发生修改时，*users* 参数将返回最后执行编辑操作的用户ID（适用于[状态 2](../../usage-api/callback-handler.md#status-2-3) 和[状态 6](../../usage-api/callback-handler.md#status-6-7) 的响应回调）。 |
