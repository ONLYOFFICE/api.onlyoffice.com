...
JSON 格式的 JWT 令牌的 payload 必须与 [config](../../usage-api/advanced-parameters.md) 具有相同的结构。

:::note
从 7.1 版本开始，将严格规范要签名的参数列表。不要忘记将下面列出的所有参数添加到您的签名中。
:::
...
调用 [insertImage](../../usage-api/methods.md#insertimage) 方法将图像插入文件时，必须添加令牌以验证参数。

JSON 格式的 JWT 令牌的 payload 必须与方法参数具有相同的结构。要签名的参数列表没有严格规定，但我们建议您指定所有发送的参数：
...
JWT 令牌的 payload（JSON 格式）必须与 [config](../../usage-api/advanced-parameters.md) 结构相同。
...
JSON 格式的 JWT 令牌的 payload 必须与方法参数具有相同的结构。待签名的参数列表没有严格规定，但我们建议您指定所有发送的参数：
...
JSON 格式的 JWT 令牌的 payload 必须与方法参数具有相同的结构。需要签名的参数列表没有严格规定，但我们建议您指定所有发送的参数：
...
当调用 [setRequestedDocument](../../usage-api/methods.md#setrequesteddocument) 方法选择要比较或组合的文档时，必须添加令牌以验证参数。
...
JSON 格式的 JWT 令牌的 payload 须与方法参数具有相同的结构。要签名的参数列表没有严格规定，但我们建议您指定所有发送的参数：
