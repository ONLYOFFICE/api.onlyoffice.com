...
<Tabs>
  ...
  async function getFolderUsers(folderId) {
    const url = `${BASE_URL}/api/2.0/people/folder/${folderId}`;
    ...
    console.log('Failed to get users for folder:', folderId, res.status, text);
    return null;
  }
  ...
  // Example usage
  (async () => {
    const folderId = '7890'; // Replace with a real folder ID

    const users = await getFolderUsers(folderId);
    console.log(`Users with access to folder ${folderId}:`);
    printUsers(users);
  })();
  ```
  ...
  def get_folder_users(folder_id):
    url = f"{BASE_URL}/api/2.0/people/folder/{folder_id}"
    ...
    print("Failed to get users for folder:", folder_id, response.status_code, response.text)
    return None
  ...
  if __name__ == "__main__":
    folder_id = "7890"  # Replace with a real folder ID

    users = get_folder_users(folder_id)
    print(f"Users with access to folder {folder_id}:")
    print_users(users)
  ```
