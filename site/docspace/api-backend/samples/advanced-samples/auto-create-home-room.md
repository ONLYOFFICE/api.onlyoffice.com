...
<Tabs>
  ...
  def get_user_info(user_id):
    ...
    data = docspace_request(f"/api/2.0/people/{safe_user_id}", "GET")
    node = get_response_node(data)
    return node if isinstance(node, dict) else None

  # Step 3: Create a personal home room
  def create_home_room(display_name):
    payload = {
  ...
  def create_base_folders(room_id):
  ...
  # Step 5: Copy starter documents from templates
  def copy_templates_to_room(room_id):
  ...
  # Step 6: Apply access rules (ACL)
  def apply_room_acl(room_id, user_id):
    entries = [
  ...
  # Step 7: Send a welcome message (placeholder)
  def send_welcome_message(user_payload, room_id):
  ...
  def handle_user_event(user_id, payload):
    ...
    if room_id is None:
      ...
      raise Exception("Failed to create home room.")
...
The script creates a room for the new user using POST [/api/2.0/files/rooms](/docspace/api-backend/usage-api/rooms/create-room)
...
For each folder name, it sends POST [/api/2.0/files/folder/:parentFolderId](/docspace/api-backend/usage-api/files/folders/create-folder)
...
For each file ID in `TEMPLATE_FILE_IDS`, it sends POST [/api/2.0/files/file/:fileId/copyas](/docspace/api-backend/usage-api/files/files/copy-file-as)
...
To configure who can access the room, the script applies room sharing rules PUT [/api/2.0/files/rooms/:roomId/share](/docspace/api-backend/usage-api/rooms/set-room-security)
