...
<Tabs>
  ...
  # Step 3: Build a safe title
  def sanitize_file_name(original_name, max_length= 100):
...
If the title is missing, the script loads it via GET [/api/2.0/files/file/:fileId](/docspace/api-backend/usage-api/files/files/get-file-info)
...
If the sanitized name differs from the original title, the script renames the file using PUT [/api/2.0/files/file/:fileId](/docspace/api-backend/usage-api/files/files/update-file) with body `{ "title": "<newTitle>" }`
