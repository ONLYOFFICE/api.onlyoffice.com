...
<Tabs>
  ...
  EMPLOYEES_ROOM_ID = 539564
  TEMPLATE_FOLDER_ID = 341029
  HR_GROUP_ID = "694513"
  ...
  def find_folder_by_title(parent_id, title):
  ...
  def get_or_create_home_folder(parent_id, folder_title):
  ...
  def copy_templates_to_home(template_folder_id, home_folder_id):
  ...
  # Step 4: Share the home folder with the user (and optional HR group)
  def share_home_folder(home_folder_id, user_id):
    share = [
  ...
  # Step 5: Send a welcome message (optional)
  def send_welcome_email(user_payload, home_folder_id):
  ...
  def handle_user_created(user_id, user_payload):
    ...
    if home_folder_id is None:
      raise Exception(f"Failed to create or find home folder for userId={user_id}")
