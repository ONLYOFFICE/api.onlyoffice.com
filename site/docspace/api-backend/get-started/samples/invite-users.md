...
## Step 1: Create users
- For each user a POST request is sent to [/api/2.0/people](../../../usage-api/add-member) with: 
    - `firstName`: User's first name.
    - `lastName`: User's last name.
    - `email`: User's email address.
- The API returns users, which is required for further operations.
...
- A POST request is sent to [/api/2.0/people/invite](../../../usage-api/invite-users)
- The API triggers an invitation email for each newly created user.
- This ensures that only new users receive an invitation, avoiding duplicate emails to existing users.
