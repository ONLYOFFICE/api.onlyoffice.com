...
For example, when sending GET requests to the [`api/2.0/people/email`](../../../../docspace/api-backend/usage-api/people/profiles/get-profile-by-email.md) or [`api/2.0/people/{userid}`](../../../../docspace/api-backend/usage-api/people/profiles/get-profile-by-user-id.md) addresses, the `LostUser` ID is used to check if a user with the specified email or name exists. If the user ID is equal to the `LostUser` ID, this user is removed from the portal and a 404 status code is returned (`User not found`).
