...
3. Configure the plugin in the terminal specifying the following settings in the dialog:
   ...
   | Version                | The current plugin version will be displayed in the DocSpace plugin settings. Plugins are cached based on their version number, so if you upload a plugin with the same version as before, DocSpace will show a warning and skip the update. Increment the version in *package.json* before each re-upload to make sure your changes are applied.                            |
