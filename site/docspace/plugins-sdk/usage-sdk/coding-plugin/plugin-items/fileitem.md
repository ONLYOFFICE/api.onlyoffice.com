...
| onClick       | func                                                                                                                | `async (item: File) => {}`                                        | Defines a function that takes the `File` object with the file data as an argument. This function can be asynchronous and can return `IMessage`, `Promise<IMessage>`, or `Promise<void>`. It will be executed when the user clicks on a file with the required extension.                                                                                                    |
...
``` ts
import {FilesSecurity, Security, type File, type IFileItem} from "@onlyoffice/docspace-plugin-sdk"
