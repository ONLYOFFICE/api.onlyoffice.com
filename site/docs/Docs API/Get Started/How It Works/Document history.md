...
2. Specify the event handler for opening the [version history](../../Usage%20API/Config/Events.md#onrequesthistory) list in the configuration script for Document Editor initialization. When the [onRequestHistory](../../Usage%20API/Config/Events.md#onrequesthistory) event is called, the [refreshHistory](../../Usage%20API/Methods.md#refreshhistory) method must be executed. This method contains document history for each document version, if the history parameter has been present for each version.
...
3. In the configuration script for Document Editor initialization, specify the event handler which will select the [version from history](../../Usage%20API/Config/Events.md#onrequesthistorydata). When the [onRequestHistoryData](../../Usage%20API/Config/Events.md#onrequesthistorydata) event is called, the [setHistoryData](../../Usage%20API/Methods.md#sethistorydata) method must be executed. This method contains the absolute URL to the file of the corresponding version.
...
4. In the configuration script for Document Editor initialization, specify the event handler which will [restore](../../Usage%20API/Config/Events.md#onrequestrestore) the file version when the user clicks the *Restore* button in the version history. When the [onRequestRestore](../../Usage%20API/Config/Events.md#onrequestrestore) event is called, the [refreshHistory](../../Usage%20API/Methods.md#refreshhistory) method must be executed to initialize version history again. This method contains document history for each document version, if the history parameter has been present for each version.
...
- [history](../../Usage%20API/Callback%20handler.md#history) - this information allows to display the time and the author for each document version when you view the document history in the side panel. Must be sent as a property changes of the object sent as the argument to the [refreshHistory](../../Usage%20API/Methods.md#refreshhistory) method.
  ...
  Where the **changes** is the *changes* from [the history object](../../Usage%20API/Callback%20handler.md#history) returned after saving the document.

  Where the **serverVersion** is the *serverVersion* from [the history object](../../Usage%20API/Callback%20handler.md#history) returned after saving the document.
