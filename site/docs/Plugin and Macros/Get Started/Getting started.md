...
1. Create a plugin folder. Follow the plugin structure described [here](../Structure/Manifest/Manifest.md). The plugin folder must contain three files required for the plugin to work: [config.json](../Structure/Manifest/Manifest.md), [index.html](../Structure/Entry%20point.md), [pluginCode.js](../Interacting%20with%20editors/Overview/Overview.md):
   ...
   Use plugin [methods](../Interacting%20with%20editors/Methods/Methods.md) and events while writing the plugin code. For example, in the **Hello world** plugin, we use the `init` and `button` events, and the `callCommand` and `executeMethod` methods.
...
2. Install a plugin [through the Plugin Manager](../Tutorials/Installing/ONLYOFFICE%20Desktop%20Editors.md#adding-plugins-through-the-plugin-manager).
