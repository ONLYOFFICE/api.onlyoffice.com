...
| ---- | ---- | ----------- |
| id | `number` | The button index in the [buttons](../fundamentals/configuration/config-json.md#variationsbuttons) array of the `config.json` file. If `id == -1`, then the plugin considers that the **Close** window cross button has been clicked or its operation has been somehow interrupted. |
